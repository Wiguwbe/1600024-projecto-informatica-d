...
int read_instance_from_file(const char* filename, puzzle_state instances[])
{
  ...
  char line[256]; // Considerando que cada linha tem no máximo 11 caracteres + o caractere nulo
...
{
  // Criamos a instancia do algoritmo A* 
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < 10; i++) {
    printf("A resolver a instancia %d...\n", i + 1);
    ...
    printf("A instancia %d foi resolvida em %.6f segundos.\n", i + 1, instance_time);
  }

  printf("Todas as instancias foram resolvidas. Tempo total: %.6f segundos.\n", total_time);
