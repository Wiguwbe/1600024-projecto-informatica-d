...
def run_executable(executable, arguments, executions=20):
    ...
    for _ in range(executions):
        output = subprocess.check_output([executable] + arguments, universal_newlines=True)
    ...
    with open(f"measurements/{output}", "wt") as f:
        for problem, instances in problems.items():
            ...
            for instance in instances:
                ...
                results_seq = run_executable(f"./{problem}/bin/{problem}", ["-r", f"./instances/{problem}_{instance}"], executions)
                ...
                for thread_num in threads:
                    ...
                    # Exhaustive search
                    row_par_exhaust = [f"\"{problem}-{instance}\"","\"paralelo - procura exaustiva\"", thread_num]
                    results_par_exhaust = run_executable(f"./{problem}/bin/{problem}", ["-r", "-n", str(thread_num), f"./instances/{problem}_{instance}"], executions)
                    row_par_exhaust.extend(calculate_average(results_par_exhaust))
                    # Calculate speed-up
                    par_exec_time = row_par_exhaust[-1]
                    row_par_exhaust.append(round(seq_exec_time/par_exec_time,3))
                    problem_measurements["paralelo - procura exaustiva"].append(row_par_exhaust)
                    
                    # First solution
                    row_par_first = [f"\"{problem}-{instance}\"","\"paralelo - primeira solução\"", thread_num]
                    results_par_first = run_executable(f"./{problem}/bin/{problem}", ["-r", "-n", str(thread_num), "-p", f"./instances/{problem}_{instance}"], executions)
                    ...
                    problem_measurements["paralelo - primeira solução"].append(row_par_first)
            ...
            for algo, measurement in problem_measurements.items():
                ...
                for instance in measurement:
                    f.write(str(';'.join(map(str, instance))).replace('.',',')+"\n")


if __name__ == '__main__':
    measure(problems, threads,50)
