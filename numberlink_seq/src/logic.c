...
void visit(state_t* current_state, state_allocator_t* allocator, linked_list_t* neighbors)
{
  ...
  for(int i = 0; i < number_link->num_pairs; i++)
  {
    if(memcmp(&(board_data.coords[i]), &(number_link->goals[i]), sizeof(coord)) == 0)
    {
      // Este par já se encontra ligado
      continue;
    }

    // Lemos as coordenadas atuais e movemos cada um dos links
    row = down = up = board_data.coords[i].row;
    col = right = left = board_data.coords[i].col;
    ...
    if(down > -1 && down < number_link->rows)
    {
      coord new_coord = { col, down };
      update_neighbors(number_link, i, new_coord, board_data, allocator, neighbors, state->matched_pairs);
    }

    // Verifica se o movimento para cima é válido
    if(up > -1 && up < number_link->rows)
    {
    ...
    if(left > -1 && left < number_link->cols)
    {
    ...
    if(right > -1 && right < number_link->cols)
    {
